<script>
  ...
  let currentTab = 0;
  let tabs = [{ name: "Photos", show: true }, { name: "Updates", show: true }];
...
{#if currentTab === 0}
          ...
          {#each photoData.photos as photo, i}
            <div
              ...
              <img
                ...
                alt="user image {i}" />
              <ChangeGalleryPicture
                modelId={photoData.photos[i].model}
                pictureId={photoData.photos[i].id} />
            </div>
          {:else}
            <p>No photos.</p>
          {/each}
  ...
  {#if photoData.photos.length < 8}
    <p class="mt-6 text-lg">
      You need to upload 8 photos. You have upload {photoData.photos.length} so
      far.
    </p>
    <div class="mt-4">
