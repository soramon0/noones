<script>
  ...
  $: galleryUploadCount = photoData.photos.length + updatesData.gallery.length;
  ...
  const modifyGalleryPhotoUpdate = (target, photoId) => {
    const file = target.files[0];

    if (file) {
      UpdatesStore.modifyGalleryPhotoUpdate(file, photoId);
    }
  };

  onMount(async () => {
    // Get user updates
    if (updatesData.gallery.length === 0) {
      await UpdatesStore.getGalleryUpdate();
    }
  });
...
{:else if showGalleryModal}
  <UploadModal
    ...
    on:file={({ detail }) => {
      UpdatesStore.createGallaryPhotoUpdate(detail);
      tabs[2] = { ...tabs[2], show: true };
      tabs = tabs;
    }}
    {photoData}
    {uiData} />
...
{#if currentTab === 0}
  <div transition:fade={{ duration: 400 }} class="mt-4 pb-8">
      ...
      <div class="relative">
        ...
        <div
          ...
          <div
            ...
            cursor-pointer sm:w-full sm:h-20 sm:top-auto sm:bottom-0 sm:bg-black
            sm:opacity-75 sm:m-0 sm:rounded-bl-none sm:rounded-br-none
            sm:rounded-md sm:translate-y-full transform transition duration-300
            ease-in-out">
            <svg
    ...
    <h1 class="text-2xl mt-6">Gallery</h1>

    <!-- Error Handling -->
    <ErrorNotifier
      errors={updatesData.galleryErrors}
      errorKey="image"
      on:clearErrors={UpdatesStore.clearGalleryError} />
    ...
    {#if galleryUploadCount < 8}
      <p class="mt-6 text-lg">
        You need to upload 8 photos. You have {updatesData.gallery.length}
        updates and {photoData.photos.length} approved so far.
      </p>
      <div class="mt-4">
...
{:else if currentTab === 2}
  {#each updatesData.gallery as photo, i}
    <div
      in:fly={{ x: 200, delay: i * 200 }}
      out:fade
      class="max-w-4xl mt-4 pb-4 border-b">
      <ErrorNotifier
        errors={updatesData.gallery[i].errors}
        errorKey="image"
        on:clearErrors={() => UpdatesStore.clearGalleryPhotoError(i)} />
        ...
        <div
          ...
          <p class="text-gray-700">
            {#if photo.message}
              {photo.message}
            {:else}
              You can change this picture for the next 24 hours after that it
              will be either approved to declined by a stuff member. In case it
              was declined, we will inform you why that is the case.
            {/if}
          </p>
          <div class="flex justify-between items-center">
            <span
              ...
              {#if !photo.accept && !photo.decline}
                Pending
              {:else if photo.accept}
                Approved
              {:else if photo.decline}Declined{/if}
            </span>
            <div class="space-x-4">
              <CancelButton
                type="button"
                ...
                on:click={() => UpdatesStore.deleteGalleryPhotoUpdate(photo.id)} />
              <UpdateButton
                type="button"
                fetching={uiData.fetching}
                on:click={() => document
                    .getElementById(`galleryPhotoUpdate-${i}`)
                    .click()} />
              <input
                type="file"
                id="galleryPhotoUpdate-{i}"
                accept="image/*"
                class="w-1 h-1 opacity-0 invisible"
                on:change={({ target }) => modifyGalleryPhotoUpdate(target, photo.id)} />
            </div>
          </div>
        </div>
      </div>
