...
<div
  class="absolute mt-4 mr-3 right-0 top-0 z-20 bg-white p-1 rounded-md"
  title="change image"
  on:click={() => document
      .getElementById(`updateGalleryPhoto-${pictureId}`)
      .click()}>
  <input
    type="file"
    id="updateGalleryPhoto-{pictureId}"
    accept="image/*"
    class="w-1 h-1 opacity-0 invisible"
    on:change={onGalleryPictureUpdate} />
  <svg
    class="{size} fill-current text-green-500 cursor-pointer"
    viewBox="0 0 20 20">
    <path
